<div class="d-flex flex-column flex-shrink-0 p-3 text-dark vh-100 admin-left">
    ...
    <ul class="nav nav-pills flex-column mb-auto">
        <li class="nav-item">
            @php
                $homeRoute = $request->route('admin_home')->getName();
                $categoryRoute = $request->route('admin_category')->getName();
                $ordersRoute = $request->route('admin_orders')->getName();
                $productsRoute = $request->route('admin_product')->getName();
                $customersRoute = $request->route('admin_customers')->getName();
            @endphp
            <a href="{{ route('admin_home') }}" class="nav-link {{ request()->is('/admin') ? 'active' : 'text-dark' }}"
