            ...
            @foreach ($orders as $order)
                <tr>
                    ...
                    <td style="width:15%"><a class="btn btn-primary"
                        ...
                        <form id="update-form" action="{{ route('admin.orders.update', $order->id) }}" method="POST"
                            class="d-none">
                            @csrf
                            <input type="hidden" name="status" value="1">
                        </form>
                        <a class="btn btn-danger" href="{{ route('admin.orders.delete', $order->id) }}"
                            onclick="event.preventDefault(); document.getElementById('delete-form').submit();">Xóa</a>
                        <form id="delete-form" action="{{ route('admin.orders.delete', $order->id) }}" method="POST"
                            class="d-none">
                            @csrf
                        </form>
                    </td>
